...
def model(flags):
  ...
  print("net_raw")
  print(net_raw.shape)
  net_mag,net_angle = tf.keras.layers.Lambda(stftLayer)((net_raw, flags.dtln_blockLen, flags.dtln_block_shift))
  print("net_mag,net_angle")
  print(net_mag.shape)
  print(net_angle.shape)
  #net_mag = tf.keras.backend.expand_dims(net_mag,1)
  #print("net_mag_expand")
  #print(net_mag.shape)
  #net_mag = tf.abs(net_spec)
  #net_phase = tf.math.angle(net_spec)
  #net_mag= InstantLayerNormalization()(tf.math.log(net_mag + 1e-7))
  ...
  net_est_mag = tf.keras.layers.Multiply()([net_mag, mask_1])
  net_est_frames_1 = tf.keras.layers.Lambda(ifftLayer)([net_est_mag, net_angle])
  
  net_encoded_frames = tf.keras.layers.Conv1D(int(flags.dtln_encoder_size),1,strides=1,use_bias=False)(net_est_frames_1)
  # normalize the input to the separation kernel
  net_encoded_frames_norm = InstantLayerNormalization()(net_encoded_frames)
  # predict mask based on the normalized feature frames
  mask_2 = dtln.DTLN(units=flags.dtln_units, num_layer = flags.dtln_num_layer, dropout = flags.dtln_dropout, mask_size = flags.dtln_encoder_size, activation = flags.dtln_activation)(net_encoded_frames_norm)
  # multiply encoded frames with the mask
  net_estimated = tf.keras.layers.Multiply()([net_encoded_frames, mask_2]) 
  # decode the frames back to time domain
  net_decoded_frames = tf.keras.layers.Conv1D(int(flags.dtln_blockLen), 1, padding='causal',use_bias=False)(net_estimated)
  # create waveform with overlap and add procedure
  net_estimated_sig = tf.keras.layers.Lambda(overlapAddLayer)((net_decoded_frames, flags.dtln_block_shift))
  
  print(net_estimated_sig.shape)
  print("DNS done")
  

  net = speech_features.SpeechFeatures(speech_features.SpeechFeatures.get_params(flags))(net_estimated_sig)
