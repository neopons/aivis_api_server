...
def model_to_tflite_uint8(sess,
                          model_non_stream,
                          flags,
                          mode=modes.Modes.STREAM_EXTERNAL_STATE_INFERENCE,
                          save_model_path=None,
                          optimizations=None,
                          experimental_new_quantizer=True,
                          representative_dataset_fuc=None,
  ...
  def representative_dataset():
    for idx in range(0, 200, 1):
      x, y = audio_processor.get_data(
          flags.batch_size, idx, flags, 0.0, 0.0, 0, 'testing', 0.0, 0.0, sess)
      yield [x.astype(np.float32)]
  
  if representative_dataset_fuc is None:
    converter.representative_dataset = representative_dataset
  else:
    converter.representative_dataset = representative_dataset_fuc
