...
def tflite_non_stream_model_accuracy(
  ...
  set_size = audio_processor.set_size('testing')//50
  ...
  inputs = []
  for s in range(len(input_details)):
    inputs.append(np.zeros(input_details[s]['shape'], dtype=np.uint8))
  ...
  for i in range(0, set_size, inference_batch_size):
    ...
    interpreter.set_tensor(input_details[0]['index'],
                           test_fingerprints.astype(np.uint8))
    ...
    out_tflite_argmax = np.argmax(out_tflite)
    print(test_fingerprints.astype(np.uint8))
    print(out_tflite)
...
def convert_model_tflite_uint8(flags,
                               folder,
                               mode,
                               fname,
                               weights_name='best_weights',
  ...
  try:
    with open(os.path.join(path_model, fname), 'wb') as fd:
      fd.write(
          utils.model_to_tflite_uint8(sess, model, flags, mode, path_model,
                                      optimizations))
  except IOError as e:
    logging.warning('FAILED to write file: %s', e)
  except (ValueError, AttributeError, RuntimeError, TypeError) as e:
