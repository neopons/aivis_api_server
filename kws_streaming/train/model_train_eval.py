...
def main(_):
  ...
  name2opt = {
      ...
      'quantize_opt_for_size_uint8_': [tf.lite.Optimize.DEFAULT],
  }
  ...
  for opt_name, optimizations in name2opt.items():
    ...
    file_name2 = 'non_stream_edgetpu.tflite'
    mode = modes.Modes.NON_STREAM_INFERENCE
    if opt_name == 'quantize_opt_for_size_uint8_':
        test.convert_model_tflite_uint8(flags, folder_name, mode, file_name,
...
if __name__ == '__main__':
  ...
  # DNS_crnn settings
  parser_DNS_crnn = subparsers.add_parser('DNS_crnn')
  DNS_crnn.model_parameters(parser_DNS_crnn)
  
  parser_crnn_lgtfb = subparsers.add_parser('crnn_lgtfb')
  crnn_lgtfb.model_parameters(parser_crnn_lgtfb) 
  
  parser_DNS_crnn_lgtfb = subparsers.add_parser('DNS_crnn_lgtfb')
  DNS_crnn_lgtfb.model_parameters(parser_DNS_crnn_lgtfb) 
