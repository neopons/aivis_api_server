...
class SpeechFeatures(tf.keras.layers.Layer):
  ...
  def _LMFB_tf_from_frames(self, inputs):
    # MFCC implementation based on TF.
    # It is based on DFT which is computed using matmul with const weights.
    # Where const weights are the part of the model, it increases model size.
    #outputs = self.windowing(inputs)
    outputs = self.mag_rdft_mel(inputs)
    outputs = self.log_max(outputs)
    return outputs
  ...
  def call(self, inputs):
    ...
    if self.params['feature_type'] == 'time':
      return outputs
    elif self.params['feature_type'] == 'mfcc_tf':
      outputs = self._mfcc_tf(outputs)
    elif self.params['feature_type'] == 'LMFB_tf':
      outputs = self._LMFB_tf(outputs)
    elif self.params['feature_type'] == 'mfcc_op':
      outputs = self._mfcc_op(outputs)
    elif self.params['feature_type'] == 'LMFB_tf_from_frames':
      outputs = self._LMFB_tf_from_frames(outputs)
    else:
