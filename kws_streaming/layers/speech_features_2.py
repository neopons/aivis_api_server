...
class SpeechFeatures(tf.keras.layers.Layer):
  ...
  def _mfcc_op(self, inputs):
    ...
    return outputs_mfcc, outputs_spec
  ...
  def call(self, inputs):
    ...
    if self.params['feature_type'] == 'mfcc_op':
      outputs_mfcc, outputs_spec = self._mfcc_op(outputs)
    else:
      raise ValueError('unsupported feature_type', self.params['feature_type'])

    outputs = self.normalizer(outputs_mfcc)

    # apply data augmentation on spectrogram:
    outputs = self.spec_augment(outputs_mfcc)
    outputs = self.spec_cutout(outputs_mfcc)
    return outputs_mfcc, output_spec
